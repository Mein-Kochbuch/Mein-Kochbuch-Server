...
public class RecipeController {
    ...
    public List<RecipePreviewDto> getAllRecipes() {
        ...
        return recipeService.getAllRecipes().stream().map(recipeMapper::convertRecipeToRecipeReviewDto).toList();
    }

    @GetMapping("{recipeId}")
    public RecipeDto getRecipeById(@PathVariable String recipeId) {
        LOG.debug("GET Recipe: " + recipeId);
        return recipeMapper.convertRecipeToRecipeDto(recipeService.getRecipeById(recipeId));
    }
    ...
    public RecipeDto addRecipe(@Valid @RequestBody RecipeCreationDto recipeCreationDto, Authentication auth) {
        ...
        return recipeMapper.convertRecipeToRecipeDto(recipeService.addRecipe(recipeCreationDto, user));
    }
}
